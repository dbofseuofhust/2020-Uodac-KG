...
class SpeckleNoise(object):
    ...
    Args:
        ...
        ratio (float, optional): probability.
    """

    def __init__(self, severity=1, ratio=0.5):
        self.severity = severity
        self.ratio = ratio

    def __call__(self, results):
        if np.random.rand() < self.ratio:
            corrupted_image = corrupt(results['img'], corruption_name='speckle_noise', severity=self.severity)
            results['img'] = corrupted_image
        return results

    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += '(severity={}, ratio={})'.format(
            self.severity, self.ratio)
        return repr_str
...
class ShiftScaleRotate(object):
    ...
    def get_aug(self, aug, min_area=0., min_visibility=0.):
        return Compose(aug, bbox_params=BboxParams(format='coco', 
                                                   min_area=min_area, 
    ...
    def __call__(self, results):
        ...
        aug = self.get_aug([ShiftScaleRotate(shift_limit=self.shift_limit, 
        ...
        annotations = {'img': img, 'gt_bboxes':boxes , 'gt_labels': labels}
        augmented = aug(**annotations)

        results['img'] = augmented['img']
        results['gt_bboxes'] = augmented['gt_bboxes']
        results['gt_labels'] = augmented['gt_labels']

        return results
        
    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += 'shift_limit={}'.format(self.shift_limit)
        repr_str += 'scale_limit={}'.format(self.scale_limit)
        repr_str += 'rotate_limit={}'.format(self.rotate_limit)
        repr_str += 'interpolation={}'.format(self.interpolation)
        repr_str += 'border_mode={}'.format(self.border_mode)
        repr_str += 'value={}'.format(self.value)
        repr_str += 'mask_value={}'.format(self.mask_value)
...
class Grid(object):
    ...
    def __call__(self, results):
        ...
        mask = torch.Tensor(np.expand_dims(mask, axis=2))
        mask = np.array(mask.expand_as(torch.Tensor(img.copy())))
    ...
    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += 'use_h={}'.format(self.use_h)
        repr_str += 'use_w={}'.format(self.use_w)
        repr_str += 'rotate={}'.format(self.rotate)
        repr_str += 'offset={}'.format(self.offset)
        repr_str += 'ratio={}'.format(self.ratio)
        repr_str += 'mode={}'.format(self.mode)
