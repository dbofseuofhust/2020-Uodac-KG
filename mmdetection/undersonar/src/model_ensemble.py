...
if __name__ == "__main__":
    ...
    ROOT_DIR = "./json/"
    CLASSES = {1:'target'}
    model1 = mmcv.load(ROOT_DIR + "qian_0406_swa_7.bbox.json")
    model2 = mmcv.load(ROOT_DIR + "qian_0326.bbox.json")
    ...
    for name in tqdm(final_names):
        for i in range(1, len(CLASSES) + 1):
            ...
            for bbox in cls_dets:
                res_line = {'image_id': name, 'category': int(i), 'bbox': [round(float(x), 2) for x in bbox[:4]],
                            'score': float(bbox[4])}
                submit.append(res_line)
    nowTime = datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S')
    out = "./json/result_{}.json".format(nowTime)
